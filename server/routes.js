...
const author = async function(req, res) {
  ...
  const name = 'Lyla Waitman';
  const pennkey = 'lwaitman';
...
const random = async function(req, res) {
  ...
  `, (err, data) => {
    if (err) {
      ...
      // also return the song title in the response
      res.json({
...
const search_songs = async function(req, res) {
  ...
  connection.query(`
    SELECT song_id, album_id, title, number, duration, 
    plays, danceability, energy, valence, tempo, key_mode, explicit
    FROM Songs
    WHERE explicit <= ${explicit} 
